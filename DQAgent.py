...
class DQAgent_split(DQAgent):
    def __init__(self,
        ...
        self.DQN = DQNetwork(
            ...
            minibatch_size=self.minibatch_size,
            dropout_prob=self.dropout_prob,
            load_path=self.load_path+'model_DQN.h5',
            logger=self.logger
        )
        ...
        self.DQN_target = DQNetwork(
            ...
            minibatch_size=self.minibatch_size,
            dropout_prob=self.dropout_prob,
            load_path=self.load_path+'model_DQN_target.h5',
            logger=self.logger
        ) # output layer 
        # Reset target DQN
        self.DQN_target.model.set_weights(self.DQN.model.get_weights())

        self.DQN_lazy = DQNetwork(
            self.actions,
            self.network_input_shape,
            learning_rate=self.learning_rate,
            discount_factor=self.discount_factor,
            minibatch_size=self.minibatch_size,
            dropout_prob=self.dropout_prob,
            load_path=self.load_path+'model_DQN_lazy.h5',
            logger=self.logger
        )

        # Target DQN used to generate targets
        self.DQN_lazy_target = DQNetwork(
            self.actions,
            self.network_input_shape,
            learning_rate=self.learning_rate,
            discount_factor=self.discount_factor,
            minibatch_size=self.minibatch_size,
            dropout_prob=self.dropout_prob,
            load_path=self.load_path+'model_DQN_lazy_target.h5',
            logger=self.logger
        ) # output layer 
        # Reset target DQN
        self.DQN_lazy_target.model.set_weights(self.DQN.model.get_weights())
        self.counter = 0
        self.policy = 0
    ...
    def get_action(self, state, testing=False, force_random=False):
        ...
        if self.counter % 1000 == 0:
            # flip policy at every 1000 steps
            if self.policy == 0:
                self.policy = 1
            elif self.policy == 1:
                self.policy = 0
    ...
    def quit(self):
        ...
        if self.load_path is None:
            ...
            self.DQN_lazy.save(append='_DQN_lazy')
            self.DQN_lazy_target.save(append='_DQN_lazy_target')